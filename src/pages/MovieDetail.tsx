...
import { ArrowLeft, Play, Lock, Crown, Star, Clock } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative h-[50vh] min-h-[350px] w-full overflow-hidden">
        ...
        <div className="absolute left-6 top-6 z-20">
          <Button variant="ghost" size="icon" onClick={() => navigate("/")} className="text-foreground bg-background/30 backdrop-blur-sm">
        ...
        <div className="flex flex-col gap-6 md:flex-row">
          ...
          >
            <img
              src={movie.poster}
              alt={movie.title}
              className="h-72 w-48 rounded-2xl object-cover shadow-card"
            />
          </motion.div>
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {movie.genre.map((g) => (
                <span key={g} className="rounded-md bg-secondary px-3 py-1 text-xs font-medium text-secondary-foreground">
                  {g}
                </span>
              ))}
            </div>
